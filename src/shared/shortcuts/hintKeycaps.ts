...
export function hintKeycaps(binding: ShortcutBinding): string[] {
  ...
  return [binding.ctrl ? 'Ctrl' : '', binding.alt ? 'Alt' : '', binding.shift ? 'Shift' : '', binding.meta ? 'Meta' : '', key].filter(Boolean);
}
