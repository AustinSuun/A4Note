...
export function PaperCaptureDetails({ paperId }: { paperId: string }) {
  ...
  return <section className="detail-section">
    ...
    {data?.files.map(file => <div key={file.id} className="binding-row"><span>{({ source_pdf: '正文 PDF', translated_pdf: '译文', version_pdf: '其他版本', supplement_pdf: '补充 PDF', supplement_file: '补充材料' } as Record<string, string>)[file.kind] || file.kind}</span><button type="button" title={file.name} onClick={() => void openCapturedPaperFile(paperId, file.id).catch(e => { if (current.current === paperId) setMessage(String(e)); })}>{file.kind === 'supplement_file' ? '所在文件夹' : '打开'} {file.name.slice(0, 28)}</button></div>)}
    <details><summary>来源与原始采集记录</summary><p className="detail-muted">保留原始元标签、字段来源证据、网页链接及文件候选。附件列表中的文件已保存；来源记录中的候选链接不代表下载成功。非PDF补充材料不自动执行或解压；缺失字段不编造。</p>{data?.snapshots.map(item => <details key={item.captureId}><summary>{item.envelope.origin === 'browser' ? '网页采集' : '本地 PDF'} · {item.envelope.sourceUrl || item.captureId}</summary><pre style={{ whiteSpace: 'pre-wrap', overflowWrap: 'anywhere', maxHeight: 360, overflow: 'auto' }}>{JSON.stringify(item.envelope, null, 2)}</pre></details>)}</details>
