...
export interface MeasurementDecorations {
  ...
  needsMixedStates: (lineFrom: number) => boolean;
...
function mixedStates(view: EditorView, from: number, model: MeasurementDecorations) {
  // Ordinary inline markup has no intrinsic-height widget. Its fully rendered
  // and fully exposed source states bound its height; building a full-document
  // decoration set for each cursor position while scrolling is quadratic.
  // Keep precise mixed-state measurement for images, formulas, tables, callouts.
  if (!model.needsMixedStates(from)) return [];
...
export function previewLayoutStability(create: (view: EditorView) => MeasurementDecorations) {
  return [previewSizes, ViewPlugin.fromClass(class {
    ...
    update(update: ViewUpdate) {
      // Decorations depend on document, cursor and configuration, not on the
      // viewport or reservation effects generated by these measurement plugins.
      if (update.docChanged || update.selectionSet || update.transactions.some(t => t.reconfigured || t.effects.some(e => !e.is(previewSizesChanged) && !e.is(mathSizesChanged)))) {
        this.measurementModel = undefined;
      }
    ...
    destroy() { this.dead = true; this.measurementModel = undefined; measureRequests.delete(this.view); this.view.dom.ownerDocument.fonts.removeEventListener('loadingdone', this.fontLoaded); this.sheet.remove(); this.view.dom.removeAttribute('data-preview-layout-owner'); }
    private writeStyles() {
    ...
    private measure(): Sizes | undefined {
      ...
      try {
        ...
        for (const node of nodes) {
          ...
          pending.push({ from, contentKey, candidates });
        }
        // One layout flush covers all candidates; subsequent reads don't
        // mutate the host or trigger another layout.
        for (const { from, contentKey, candidates } of pending) {
          const height = Math.max(...candidates.map(line => line.getBoundingClientRect().height));
