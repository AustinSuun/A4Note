...
## 集成复核：合入 main 后用指定真实论文实测（会话 A，21:5x）

- 环境：`.worktrees/integration-check-arena` 分离检出 `main 0909744`（含 35496a06、fb5e3f2f、24ea34e5 等全部合并），静态 debug 构建 `arena-f5`，样本为用户放回的 `docs/pdf_test/2505.13447v1.pdf` 与 `docs/pdf_test/2505.13447v1-仅译文.pdf`，驱动 `.tmp/f5/f5-selection-driver.mjs`（DOM 矩形 + 截图像素扫描双重测量，`measurements.json`）。
- 每组以运行自身高度归一化，列出拖选中色带与落地高亮 top/bottom 差值，以及被透明化的浏览器原生选区框本来会有的差值（即修复前的跳变）：

| 运行 | 字号 px | DOM 差值 top/bottom | 像素差值 top/bottom | 原生选区框差值 top/bottom |
| --- | --- | --- | --- | --- |
| 英文标题 100% | 17.2 | 0 / 0 | 0 / 0 | 0.47 / 0.43 |
| 英文正文 100% | 10.0 | 0 / 0 | 0 / 0 | 0.52 / 0.38 |
| 英文标题 150% | 25.8 | 0 / 0 | 0 / 0 | 0.44 / 0.45 |
| 英文作者行 150% | 14.9 | 0 / 0 | 0 / 0 | 0.50 / 1.03 |
| 英文标题 200% | 34.4 | 0 / 0 | 0 / 0 | 0.44 / 0.46 |
| 中文标题行 100% | 9.9 | 0 / 0 | 0 / 0 | 0.52 / 0.40 |
| 中文正文 100% | 9.9 | 0 / 0 | 0 / 0 | 0.52 / 0.40 |

- 同一页上黄色单层带采样 `#ffeeb7`，与蓝色重叠区采样 `#ffeeb7`（相对亮度 0.858 相同）：重叠不叠加变深。合成旋转页的竖向临时色带与落地矩形逐像素相同。
- 未计入的组：200% 正文行与中文脚注行在这份论文里落在含上标/公式的混排行，驱动的行分组把两行并成一组导致落地记录含多段，属测量脚本选行问题，不是几何差异（其像素差值同样为 0/0）。
- 同一构建上另跑 F1 旋转页驱动 62/63（唯一失败为已知的位图占位 canvas 探针抖动）与 F3 跨页驱动 22/22。
