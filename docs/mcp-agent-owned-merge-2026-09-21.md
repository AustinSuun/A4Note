...
用户选择先完成流程修复再打本地包，并明确允许仅豁免main中指定的未跟踪报告：原地保留，不提交、不删除，合并前后检查SHA256及工作区状态。其他变化仍停止。

已在独立分支整合main34738d6，合并提交9008c53；状态JSON冲突保留双方记录解决。任务专项86通过1跳过、验收109断言、编辑冲突14断言、资源依赖校验、build、架构和状态检查通过。升级本地打包版本至0.1.25（现有本地latest为0.1.24，旧产物保持不动）。新版本完整npm run verify通过，Rust201通过5忽略。没有宣称原生GUI或现有真实任务库归档已验证。

下一步：提交版本变更后执行带状态与文件哈希校验的ff-only本地main合并；仅合并成功后运行标准package:windows，输出在本独立工作区artifacts/windows/latest，不安装、不重启服务、不推送或发布。
