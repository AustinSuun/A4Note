# A4 Note Agent 状态
- capture-readable-pdf-arena：用户确认只为之后浏览器插件新采集的 PDF 生成易读文件名，已实现“论文标题 - 原文/其他版本/补充材料 - SHA-256 前16位.pdf”；标题缺失只回退可信 arXiv/DOI 或“论文”，过滤 Windows 非法字符/双向控制符、限制文件名长度不超过原哈希名。完整哈希仍用于验证/去重，内部 paper/capture ID 目录、旧文件及非PDF附件不迁移。译文导入仍复制至同一论文父目录 `translated.<语言或manual>.<fileId>.pdf`，不进入 captures 子目录、不移动原文件。定向 Rust 8/8、隔离工作树完整 `npm run verify` 退出0（Rust 220通过/5忽略）；无真实资料库改动、安装、打包、推送或发布。目标本地 main，交付与限制详见 docs/mcp-capture-readable-pdf-filenames-arena-2026-09-23.md。更新：2026-09-23T14:05:00+08:00。
