# A4 Note Agent 状态
- layer-toolbar-height-xunzhou：巡舟完成 f3878879 阅读器标注工具栏图层入口等高/左侧残留/撑高修复。根因由隔离 dev:live 实例（xunzhou-toolbar，1461/CDP9271，DEV 身份条原生核验）真实量测锁定：入口规则 `flex: 0 0 64px` 落在遗留 reader.css 造成的列方向 slot 主轴上被解释为高度（入口 64×64、同排 36×36、工具栏被撑至 69.6px），另有 border-left 1px + margin-left 3px + 圆角 8px(同排 9px) 旧容器残留。修复删除 flex-basis/左边框/左外边距/独立圆角并把 dock 内 slot 方向钉死 row；修复后入口 64×36 与同排高度差/顶边差 0px，工具栏 41.6px，125%/150% 缩放、1/1 2/2 12/12、hover/focus/弹窗展开、深浅主题、460px 窄窗全部稳定。新增 `test:layer-toolbar-geometry` 真实应用级联（含遗留 reader.css）几何回归 42/42，旧布局红测 22 通过/20 失败；picker 43/43、popover 166/166、layers 55、tsc/build 0、diagnostics 0；npm run verify 除 test:reader、test:ui-state 两步既有失败（干净基线复现同一断言，qinglan 3781ddf 文本工具简化后 verify 契约未同步，8a3b1ed 已记录 blocker）外全部通过（Rust 215/0/5 ignored）。原生功能探针 14/14（弹窗/选层/眼睛计数/Esc/外部点击/Tab 顺序/Enter/焦点描边），pageerror 0；修复前后各 9 张原生截图与量测 JSON 已上传任务卡。已合入本地 main，SHA 见任务卡交付记录。详见 docs/mcp-layer-toolbar-height-xunzhou-2026-09-22.md。
...
更新时间：2026-09-22T20:51:17+08:00
