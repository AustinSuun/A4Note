...
- 分支：`feat/reader-note-panel-polish-arena`（worktree `.worktrees/note-panel-polish-arena`），基线 main `fc6be92`；`4d6411d`（实现 + 回归）、`4d86f75`（下拉锚点）、`6f6064c`（首版文档）、`803a381`/`ed0ff36`（spec 12 补充）已 ff 合入本地 main；本文档修订提交见任务卡。
- 期间主仓库根目录被其他 Agent 切到 `fix/markdown-fast-scroll-arena`（其分支含本任务前三次提交），本任务改用临时 worktree `.tmp/arena-polish/wt-main` 对 `main` 做 ff 合并，spec 12 之后的 after 截图改从 worktree 自己的 dev:live 实例 `arena-polish-wt` 采集。
...
### 2.7 spec 12 补充：全屏/侧栏统一、收起态入口可见性与尺寸（`803a381`、`ed0ff36`）
- 收起态右缘「笔记」入口从 22×88 放大到 **30×96**：底色 `color-mix(var(--accent) 16%, var(--surface))`（浅色主题实测 `srgb(.891,.917,.905)`，非白）、边框 `color-mix(var(--accent) 44%, var(--line))`、外阴影，图标 16px、竖排文字 `--ui-control-font-size` 加粗、字色 `--accent-strong`；悬停 24% 底色 + 深边框，按下 34%，focus-visible 外描边；`right: 0` 贴边、轮廓完整在窗口内。
- 专注写作 / 窄窗覆盖态的左缘「返回论文」把手改为同一语言的镜像标签（28×76，chevron 常显），与展开态握条、收起态书签形成一套：常态/悬停/聚焦/按下与方向反馈一致，点击/拖宽/右键长按菜单/键盘不变。
- 侧栏 → 专注写作：`.reader-main-workspace` 的 `visibility` 延迟到过渡结束再隐藏（`transition: visibility 0s var(--note-transition-duration)`），FLIP 放大过程中正文一直在下方，反向立即可见；配合 FLIP 使全屏 ⇄ 侧栏双向连续。
- 所有笔记形态（分屏/悬浮/写作/覆盖）顶部不再显示常态「已保存」（`.note-save-state.saved { display: none }`），saving / dirty / error 反馈保留。
- 标题触发区紧凑靠左、按钮组靠右、下拉字级统一的规则本就作用于 `.reader-workspace-drawer`，三种形态一致（after 分屏/悬浮/写作截图均可见）。
...
- `scripts/verify-note-workbench-browser.mjs`：缩放步骤改用 se 角柄，新增 nw 角柄对角固定、四角柄/无旧按钮/无左缘拖宽条、把手握条形态、收起把手 30×96 贴右边界且有色底/边框/阴影/字号 ≥13px/图标 ≥16px 的断言（95/95）；`scripts/verify-note-workbench.mjs` 新增 5 项静态守卫（66/66）；`scripts/fixtures/note-edge-host.tsx` 改用新控件。

## 4. 运行时证据（1800×1000，DEV 状态条；before = fc6be92 于根目录实例 `arena-polish` 1461/9361；after = ed0ff36 于 worktree 实例 `arena-polish-wt` 1465/9365，另存 `after-4d6411d/`、`after-4d86f75/`、`after-803a381/`）

- 截图与 result.json：`.tmp/shots/note-panel-polish/{before,after}/`（35 / 47 张：00 基线与收起把手常态/悬停/按下、01 轨道滑开 + 90ms 中间帧、02 展开把手悬停、03 文档下拉、04 分屏→悬浮 + 70ms 中间帧、04b/04c 分屏⇄专注写作 + 中间帧与左缘返回标签、05 角柄悬停、06/07 右下/左上角缩放、08 小卡片、09 悬浮→分屏、10 收起 + 中间帧、11 快捷键弹出 + 中间帧、12 收回、13/14 窄窗/宽窗、15 PDF 放大（Ctrl+滚轮，407%）下的收起把手与分屏、matrix-{light,dark}-{100,125,150}-{split,floating,picker}）。before/after 均 0 pageerror、0 console error。
- 帧时序取自机器空闲时的 `after-4d6411d/`、`after-4d86f75/` 采样（最终 `after/` 采集时其他 Agent 正在跑 cargo 构建，FLIP 场景出现 100–167ms 长帧，属机器负载，同一场景在空闲采样中为 60fps；轨道场景在最终采样中仍为 16 帧 16.66/16.8ms）。轨道滑开（after 10b 再次打开）：`entered` 后 260ms 窗口 16 帧、均值 16.66ms、最大 17.1ms，轨道 203→360→469→539→583→609→626→635→645，PDF 列同步 13 个值，抽屉宽度恒为 645，把手 left 1555→1401→1293→1224→1181→1154；收起（10）16 帧 16.66/16.8ms、轨道 638→357→…；首次打开（01）含一帧 117ms 冷启动布局（before 同样存在）。before 的同一场景轨道值恒为 534、只有抽屉自身 28px 位移。
...
- 收起态入口（after，白色 PDF 放大到 407% 仍在窗口内）：30×96、right 贴外壳右缘（1742–1772 / 1772）、底色 `srgb(.891,.917,.905)`、边框 .8px、阴影、字色 accent-strong；悬停 `srgb(.837,.876,.858)`；before 为 22×88、底色 `srgb(.921,.941,.924)` 近白无强调。专注写作态左缘返回标签 28×76 同色系。分屏 → 专注写作 70ms 中间帧：`matrix(.9067, …, 269.5, 41.4)` 全屏面板从侧栏盒放大、正文仍可见；反向 `matrix(.9067, …, 404.3, 82.9)`。
...
- worktree：`npx tsc --noEmit` 0；`npm run build` 通过；`test:note-workbench` 65/65、`test:reader-note-sidebar` 16/16、`test:reader`、`test:reader-helpers`、`test:ui-state`、`test:architecture` 通过、`test:reader-note-sidebar-browser` 通过、`test:note-workbench-browser` 94/94、`test:note-enter-motion-browser` 68/68；`get_diagnostics` 0；完整 `npm run verify` 见状态条目。
...
- dev:live 实例 `arena-polish`（根目录，1461/CDP 9361）与 `arena-polish-wt`（worktree，1465/CDP 9365）交付时仍在运行，按规则未由 agent 结束，请用户验收后关闭。
