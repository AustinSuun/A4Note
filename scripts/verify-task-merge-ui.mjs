...
const root=process.cwd(), scratch=fs.mkdtempSync(path.join(os.tmpdir(),'merge-ui-'));
const evidence=path.resolve('.tmp/task-merge-ui');fs.mkdirSync(evidence,{recursive:true});
...
try{
 const entry=path.join(root,'.tmp/merge-ui-entry.tsx');
 ...
 for(let i=0;i<100;i++){if(await evaluate('document.body.innerText.includes("main 合并")'))break;await pause(100);}
 assert.ok(await evaluate('document.body.innerText.includes("未提交修改")'));
 assert.ok(await evaluate('document.body.innerText.includes("新负责人")'));
 assert.ok(await evaluate('document.body.innerText.includes("未推送")'));
 await evaluate('[...document.querySelectorAll("button")].find(b=>b.textContent==="检查实际效果").click()');
 ...
 assert.ok(await evaluate('document.body.innerText.includes("1234567890abcdef")'));
 fs.writeFileSync(path.join(evidence,'merge-blocked.png'),Buffer.from((await rpc('Page.captureScreenshot',{format:'png'})).data,'base64'));
 fs.writeFileSync(path.join(evidence,'result.json'),JSON.stringify({passed:true,checks:6,scope:'production components in isolated Chrome fixture; not native app'}));
 console.log('6 browser checks passed; '+evidence);
