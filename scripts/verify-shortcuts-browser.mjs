...
try {
  ...
  await row.getByRole('button',{name:'更改',exact:true}).click();await page.keyboard.press('Delete');await page.getByRole('button',{name:'保存绑定'}).click();check(await key('Control+u'),[],'Delete clears mapping');
  ...
  await row.getByRole('button',{name:'重置',exact:true}).click();await page.getByRole('button',{name:'保存绑定'}).click();
  ...
  await row.getByRole('button',{name:'更改',exact:true}).click();await side();await page.getByRole('button',{name:'保存绑定'}).click();
