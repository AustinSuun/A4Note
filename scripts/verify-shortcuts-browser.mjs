...
try {
  ...
  await row.getByRole('button',{name:'更改',exact:true}).click();await page.keyboard.press('Delete');await page.getByRole('button',{name:'保存更改'}).click();check(await key('Control+u'),[],'Delete clears mapping');
  await row.getByRole('button',{name:'更改',exact:true}).click();await page.keyboard.press('Escape');check(await page.locator('.shortcut-recorder').count(),0,'Escape cancels recorder');
  await row.getByRole('button',{name:'更改',exact:true}).click();await page.evaluate(()=>window.dispatchEvent(new Event('blur')));check(await page.locator('.shortcut-recorder').count(),0,'blur cancels recorder');
  await page.getByRole('checkbox',{name:'按物理键位识别',exact:false}).check();
  await row.getByRole('button',{name:'更改',exact:true}).click();await page.keyboard.press('Control+Shift+F9');await page.locator('.shortcut-recorder').getByRole('button',{name:/保存/}).click();
  ...
  await row.getByRole('button',{name:/^恢复.+默认快捷键$/}).click();await page.getByRole('button',{name:'保存更改'}).click();
  ...
  await row.getByRole('button',{name:'更改',exact:true}).click();await side();await page.getByRole('button',{name:'保存更改'}).click();
