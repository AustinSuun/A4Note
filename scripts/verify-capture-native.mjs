...
const availability=[];const unsubscribe=bridge.subscribeDisconnect(e=>availability.push(e));
...
ports[0].onMessage.emit({id:ports[0].sent.at(-1).id,ok:false,error:{code:'access_denied',message:'用户拒绝授权'}});await denied;assert.equal(availability.at(-1).code,'access_denied');assert.equal(ports.length,1,'Authorization denial must retain the live host transport');
