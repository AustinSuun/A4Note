...
function App(){const[notes,setNotes]=useState(initial);const paper={paperId:'paper-a',title:'Paper',notes,annotations:[]};return <div className="reader-workspace-shell"><aside className="reader-workspace-drawer notes-active" style={{width:360,height:680}}><div className="workspace-panel-content"><div className="reader-retained-note"><MarkdownNotePanel paper={paper} draftPatch={null} onDraftPatchConsumed={()=>{}} onSave={async n=>setNotes(x=>x.map(v=>v.id===n.id?{...v,...n}:v))} onCreateNote={async()=>{const id='note-'+(notes.length+1);setNotes(x=>[...x,{id,title:'阅读笔记 '+(x.length+1),content:''}]);return id}} onNavigateAnnotation={()=>{}}/></div></div></aside></div>};createRoot(document.getElementById('root')).render(<App/>);`);
...
try{server=await createServer({configFile:false,root,cacheDir:path.join(hostDir,'cache'),optimizeDeps:{entries:['.tmp/reader-note-sidebar-browser/index.html']},plugins:[react()],resolve:{alias:[{find:path.resolve(root,'src/platform/library/summaries.ts'),replacement:path.join(hostDir,'mock-summaries.ts')},{find:path.resolve(root,'src/platform/library/noteDocuments.ts'),replacement:path.join(hostDir,'mock-sessions.ts')}]},server:{host:'127.0.0.1',port:0,watch:null},logLevel:'error'});await server.listen();
 ...
 const triggerWidth=await page.locator('.note-document-trigger').evaluate(el=>el.getBoundingClientRect().width);if(triggerWidth<40)throw Error('narrow container collapsed document picker: '+triggerWidth);
