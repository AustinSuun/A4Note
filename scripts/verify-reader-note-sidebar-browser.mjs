...
try{server=await createServer({configFile:false,root,cacheDir:path.join(hostDir,'cache'),plugins:[react()],resolve:{alias:[{find:path.resolve(root,'src/platform/library/summaries.ts'),replacement:path.join(hostDir,'mock-summaries.ts')},{find:path.resolve(root,'src/platform/library/noteDocuments.ts'),replacement:path.join(hostDir,'mock-sessions.ts')}]},server:{host:'127.0.0.1',port:0},logLevel:'error'});await server.listen();
