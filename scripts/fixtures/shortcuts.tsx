...
function Harness() {
  ...
  return <main style={{padding:20, maxWidth:850}}>
    ...
    <nav><button onClick={()=>setScene('library')}>Library</button><button onClick={()=>setScene('reader')}>Reader</button><button onClick={()=>setModal(!modal)}>Modal</button></nav>
    ...
    <div ref={root} style={{display:scene==='reader'?'block':'none'}}>
      <div>{['cursor','highlight','underline','area','text','ink','eraser','rect','arrow'].map(t=><button key={t} {...props('reader.tool.'+t,t)}>{t}</button>)}</div>
