...
const evidence = path.join(root, '.tmp/multi-project');
fs.mkdirSync(evidence, { recursive: true });
const scratch = fs.mkdtempSync(path.join(os.tmpdir(), 'a4-multi-project-'));
...
const click = text => evaluate(`(()=>{const bs=[...document.querySelectorAll('button')];const b=bs.find(e=>(e.textContent.trim()===${JSON.stringify(text)}||e.getAttribute('aria-label')===${JSON.stringify(text)}));if(!b)throw Error('Button missing: '+${JSON.stringify(text)});b.click()})()`);
...
<script type="module" src="/entry.js"></script>`);
  ...
  const entry = path.join(root, '.tmp/multi-project/entry.tsx');
...
`;
  ...
  assert.deepEqual(await evaluate(`[...document.querySelectorAll('.tb-stage-switch button')].map(b=>b.textContent.trim())`), ['总览','任务队列','正在进行','待检查效果','已归档']); checks++;
  ...
  for (const label of ['任务队列','正在进行','待检查效果','已归档','总览']) {
