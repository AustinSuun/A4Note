...
fn native_host_real_process_roundtrip(){
    ...
    for (id,op,payload) in [(1,"hello",json!({})),(2,"request_access",json!({})),(3,"submit",super::model::fixture()),(4,"list_tasks",json!({}))]{
        super::native_protocol::write_frame(&mut input,&json!({"id":id,"operation":op,"payload":payload})).unwrap();input.flush().unwrap();let response=receiver.recv_timeout(Duration::from_secs(10)).expect("host framed response timeout");assert_eq!(response["id"],id);assert_eq!(response["ok"],true,"{response}");if id==1{assert_eq!(response["result"]["authorized"],false);}if id==4{assert_eq!(response["result"]["tasks"].as_array().unwrap().len(),1);}
    }
